...
layui.define(function (exports) {
  ...
  const Util = {
     ...
     * 渲染下拉框option
     * @example
     * renderOption('select[name=xxx]', status_list, 'status_id', 'status_name)
     */
    renderOption: (obj) => {
      const { selector, list, value = 'id', text = 'name' } = obj
      if (!selector || $(selector).length === 0) {
        throw new Error('selector出错')
      }
     ...
     * 渲染面包屑
     * @example
     * renderBread('#C-bread', [['/','首页']])
     */
    renderBread: (selector, arr) => {
      if (!selector || $(selector).length === 0) {
        throw new Error('selector出错')
      }
      if (!Util.isArray(arr)) {
        throw new Error('需要二维数组')
      }
      let content = ''
      let arrLen = arr.length
      layui.each(arr, function (index, link) {
        if (!Util.isArray(link)) {
          throw new Error('需要二维数组')
        }
        if (link[0]) {
          content += `<a lay-href="${link[0]}">${link[1]}</a>`
        } else {
          content += `<a><cite>${link[1]}</cite></a>`
        }
      })
    ...
    msgOk: (msg, cb) => {
      layer.msg(
        ...
        typeof cb === 'function' ? setTimeout(cb, 800) : null
      )
    },
    ...
    /**
     * 传入form里的字段，obj.field，返回url，用来：location.hash = Util.addUrlParams(field)
     * @example
     * addUrlParams({name:'xx'})//form里面的字段
     * @returns object
     */
    addUrlParams: (field) => {
      let search = layui.router().search
      let newSearch = Object.assign(search, field)
      let strSearch = ''
      for (let item in newSearch) {
        if (newSearch[item]) {
          strSearch += '/' + item + '=' + newSearch[item]
        }
      }
      let path = '/' + layui.router().path.join('/')
      let url = path + strSearch
      return url
    },
    /**
     * 类似layui.router().search，但会自动解码中文
     * @example
     * Util.getUrlParams()
     */
    getUrlParams: (field) => {
      let search = layui.router().search
      if (field) {
        search = Object.assign(search, field)
      }
      for (let key of Object.keys(search)) {
        if (search[key]) {
          search[key] = decodeURI(search[key] + '')
        }
      }
      return search
    },
     ...
     * 刷新左侧菜单
     */
    menuRefresh: function (toUrl) {
      let that = layui.view('TPL_layout')
    ...
    isMobile: function () {
      return !!navigator.userAgent.match(
        /(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|UCWeb|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/i
      )
    },
    ...
    disable: function (selector) {
      $(selector).prop('disabled', 'disabled').addClass('layui-disabled')
    },
    ...
    enable: function (selector) {
      $(selector).removeProp('disabled').removeClass('layui-disabled')
    },
    ...
    show: function (selector) {
      $(selector).removeClass('layui-hide')
    },

    /**
     * 隐藏表单元素，参数是字段的name值，<input name='field1'>
     * @example
     * readOnlyField(['field1','field2'])
     */
    hideField: function (arr) {
      for (let item of arr) {
        $('[name=' + item + ']')
          //.parents('.layui-inline')
          .parent()
          .parent()
          .addClass('layui-hide')
      }
    ...
    /**
     * 让表单元素变成只读文本，参数是字段的name值，<input name='field1'>
     * @example
     * readOnlyField(['field1','field2'])
     */
    readOnlyField: function (arr) {
      for (let item of arr) {
        let value = $('[name=' + item + ']').val()
        $('[name=' + item + ']')
          .parent()
          .html(value)
          .addClass('layui-read')
     ...
     * readOnlyForm('#id')
     */
    readOnlyForm: function (selector) {
      $(selector)
        .find(':input')
        .each(function () {
          let value = $(this).val()
          ...
          $(this).parent().html(value).addClass('layui-read')
        })
    },
    ...
    */
     ...
     * @returns function
     */
    throttle: function (func, delay = 1000) {
      var timer = null
