...
export class KTDataTable<T extends KTDataTableDataInterface> extends KTComponent implements KTDataTableInterface {
  ...
  private _createPaginationButtons(paginationContainer: HTMLElement): void {
    ...
    if (pageMoreEnabled) {
      ...
      for (let i = range.start; i <= range.end; i++) {
        paginationContainer.appendChild(
          createButton(number.text.replace('{page}', i.toString()), `${number.class}${currentPage === i ? ' active disabled' : ''}`, currentPage === i, () =>
    ...
    // Add Next Button
    paginationContainer.appendChild(
      createButton(next.text, `${next.class}${currentPage === totalPages ? ' disabled' : ''}`, currentPage === totalPages, () =>
  ...
  protected _checkboxHandler(): void {
    ...
    KTEventHandler.on(document.body, this._config.attributes.checkbox, 'input', (event: Event) => {
  ...
  public setFilter(filter: KTDataTableColumnFilterInterface): KTDataTable<T> {
    this._config._state.filters = [...(this.getState().filters || []).filter((f) => f.column !== filter.column), filter]
    return this
  }
